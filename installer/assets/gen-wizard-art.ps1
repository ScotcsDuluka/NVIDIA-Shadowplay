...
foreach ($i in 1..5) {
    ...
    $g.DrawArc($glow, $mx - $mR - $i, $my - $mR - $i, ($mR + $i) * 2, ($mR + $i) * 2, 115, 290)
}
$ring = New-Object Drawing.Pen($green, 6)
$ring.StartCap = 'Round'; $ring.EndCap = 'Round'
$g.DrawArc($ring, $mx - $mR, $my - $mR, $mR * 2, $mR * 2, 115, 290)
$slit = New-Object Drawing.Pen($white, 3)
$slit.StartCap = 'Round'; $slit.EndCap = 'Round'
$g.DrawLine($slit, $mx - 10, $my, $mx + 10, $my)

$fBrand = $null; $fSub = $null; $fTiny = $null
try   { $fBrand = New-Object Drawing.Font('NVIDIA Sans Bd', 16, [Drawing.FontStyle]::Bold) }
catch { $fBrand = New-Object Drawing.Font('Segoe UI', 16, [Drawing.FontStyle]::Bold) }
try   { $fSub = New-Object Drawing.Font('NVIDIA Sans', 10) } catch { $fSub = New-Object Drawing.Font('Segoe UI', 10) }
try   { $fTiny = New-Object Drawing.Font('NVIDIA Sans', 8.5) } catch { $fTiny = New-Object Drawing.Font('Segoe UI', 8.5) }

$g.DrawString('NVIDIA', $fBrand, [Drawing.Brushes]::White, ($mx + 38), ($my - 24))
$g.DrawString('S H A D O W P L A Y', $fSub, (New-Object Drawing.SolidBrush($green)), ($mx + 40), ($my + 4))

# green hairline under the header row
$hair = New-Object Drawing.Pen([Drawing.Color]::FromArgb(80, 118, 185, 0), 1)
$g.DrawLine($hair, 44, 108, $W - 44, 108)

# green floor glow
$fr = [Drawing.Rectangle]::new(0, $H - 110, $W, 110)
