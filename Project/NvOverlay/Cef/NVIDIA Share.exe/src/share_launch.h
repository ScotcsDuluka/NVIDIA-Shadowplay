...
struct ShareLaunchParams {
  ...
  int backend_port = 0;          // --backend-port=N: NvBackend origin mode
                                 // (QUERY_WIN_NODE_INFO returns N; nav URL
                                 // points at the backend, which serves the
                                 // osc frontend same-origin per its boot
                                 // contract — Backend/index.js)
};
...
struct ShareHostContext {
  int http_port = 0;
  int backend_port = 0;          // 0 = standalone (static server is the page origin)
