                                          ...
                                          AddHandler _delayTimer.Tick, Sub()
                                                                           ...
                                                                           StartSlide(Notifier_black, Me.Width, Me.Width - 300, 300,
                                                                               Sub()
                                                                                   ...
                                                                                   Notifier_Sub3.Reveal(Me.Left, Me.Top, Me.Notifier_black.Left)
                                                                                   FadeInShadow()
                                                                               End Sub)
                                                                       End Sub
                                          _delayTimer.Start()
...
#Region "T30 RaiseStack - Unit Move / Z-Order"
    ...
    Private Sub ApplySlideX(panel As Control, newX As Integer)
        ...
        If Notifier_Sub3 IsNot Nothing AndAlso Not Notifier_Sub3.IsDisposed AndAlso Notifier_Sub3.Visible Then
            ...
            If Notifier_Sub3.Left <> riderX Then
                moves.Add(New PendingMove With {.Hwnd = Notifier_Sub3.Handle, .X = riderX, .Y = Notifier_Sub3.Top})
            End If
        End If
        If Shadow3 IsNot Nothing AndAlso Not Shadow3.IsDisposed AndAlso Shadow3.Visible Then
            If Shadow3.Left <> Me.Left OrElse Shadow3.Top <> Me.Top Then
                moves.Add(New PendingMove With {.Hwnd = Shadow3.Handle, .X = Me.Left, .Y = Me.Top})
            End If
        End If
        FlushMoves(moves)
    End Sub
    ...
    Private Sub ApplyUnitY(newTop As Integer)
        ...
        End If
        If Notifier_Sub3 IsNot Nothing AndAlso Not Notifier_Sub3.IsDisposed AndAlso Notifier_Sub3.Visible AndAlso Notifier_Sub3.Top <> newTop Then
            moves.Add(New PendingMove With {.Hwnd = Notifier_Sub3.Handle, .X = Notifier_Sub3.Left, .Y = newTop})
        End If
        If Shadow3 IsNot Nothing AndAlso Not Shadow3.IsDisposed AndAlso Shadow3.Visible AndAlso
           (Shadow3.Top <> newTop OrElse Shadow3.Left <> Me.Left) Then
            moves.Add(New PendingMove With {.Hwnd = Shadow3.Handle, .X = Me.Left, .Y = newTop})
        End If
        FlushMoves(moves)
    End Sub
    ...
    Public Sub FadeInShadow()
        Try
            If Shadow3 Is Nothing OrElse Shadow3.IsDisposed OrElse Shadow3.Disposing Then Return
            ' Already steady at full opacity and no fade running -> nothing to do.
            If Shadow3.Visible AndAlso Shadow3.Opacity >= 1R AndAlso _shadowFade Is Nothing Then Return
            If _shadowFade IsNot Nothing Then
                _shadowFade.Stop()
                _shadowFade.Dispose()
                _shadowFade = Nothing
    ...
    Public Sub RaiseUnit()
        Try
            Dim hasShadow As Boolean = Shadow3 IsNot Nothing AndAlso Not Shadow3.IsDisposed AndAlso
                                       Shadow3.Visible AndAlso Shadow3.IsHandleCreated
            Dim hasRider As Boolean = Notifier_Sub3 IsNot Nothing AndAlso Not Notifier_Sub3.IsDisposed AndAlso
                                      Notifier_Sub3.Visible AndAlso Notifier_Sub3.IsHandleCreated
            ...
            If h <> IntPtr.Zero Then
                ...
                If hasRider Then
                    h = DeferWindowPos(h, Notifier_Sub3.Handle, prev, 0, 0, 0, 0,
                                       SWP_NOSIZE_FLAG Or SWP_NOMOVE_FLAG Or SWP_NOACTIVATE_FLAG)
                    If h <> IntPtr.Zero Then prev = Notifier_Sub3.Handle
                End If
                If h <> IntPtr.Zero Then
                ...
                If h <> IntPtr.Zero AndAlso hasShadow Then
                    h = DeferWindowPos(h, Shadow3.Handle, prev, 0, 0, 0, 0,
                                       SWP_NOSIZE_FLAG Or SWP_NOMOVE_FLAG Or SWP_NOACTIVATE_FLAG)
                End If
                If h <> IntPtr.Zero AndAlso EndDeferWindowPos(h) Then Return
            End If
        Catch ex As Exception
            Debug.WriteLine("[Notifier3] RaiseUnit defer error: " & ex.Message)
        End Try
