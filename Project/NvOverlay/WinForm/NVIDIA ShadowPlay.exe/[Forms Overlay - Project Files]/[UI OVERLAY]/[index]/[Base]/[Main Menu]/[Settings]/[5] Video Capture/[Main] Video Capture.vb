...
#Region "Form Events"
    ...
    Public Sub Base_RecordingsSet_Load(sender As Object, e As EventArgs) Handles Me.Load
        UpdateCaptureCursorLabel(ReadCaptureCursorConfig())
    ...
    ''' <summary>Mouse toggle for recording (owner 2026-09-25): the same
    ''' entry point as the Alt+F9 hotkey, exposed as a clickable control on
    ''' this page. The label alternates REC / STOP on each click; the engine
    ''' owns the authoritative recording state.</summary>
    Private Sub Record_Toggle_Click(sender As Object, e As EventArgs) Handles Record_Toggle.Click
        Try
            Base.ToggleRecording()
            Record_Toggle.Text = If(Record_Toggle.Text = "R E C", "S T O P", "R E C")
        Catch ex As Exception
            Debug.WriteLine("Record_Toggle_Click Error: " & ex.Message)
        End Try
    End Sub

    ' ── Capture Cursor on/off (owner 2026-09-25) ────────────────────────
    ' Persists engine.json "CaptureCursor" (DdagrabBackend reads it at
    ' engine start). The label mirrors the current persisted state.
    Private Function ReadCaptureCursorConfig() As Boolean
        Try
            Dim path As String = AppLayout.P("Config", "engine.json")
            If File.Exists(path) Then
                Using doc As System.Text.Json.JsonDocument =
                    ...
                    If doc.RootElement.TryGetProperty("CaptureCursor", el) Then
    ...
    Private Sub WriteCaptureCursorConfig(enabled As Boolean)
        Try
            Dim path As String = AppLayout.P("Config", "engine.json")
            Dim node As System.Text.Json.Nodes.JsonNode
            If File.Exists(path) Then
                node = System.Text.Json.Nodes.JsonNode.Parse(File.ReadAllText(path))
            Else
                node = New System.Text.Json.Nodes.JsonObject()
            End If
            node("CaptureCursor") = enabled
            File.WriteAllText(path, node.ToJsonString(
                New System.Text.Json.JsonSerializerOptions With {.WriteIndented = True}))
        Catch ex As Exception
            Debug.WriteLine("WriteCaptureCursorConfig Error: " & ex.Message)
        End Try
    End Sub

    Private Sub UpdateCaptureCursorLabel(enabled As Boolean)
        Capture_Cursor.Text = If(enabled, "C U R S O R  O N", "C U R S O R  O F F")
        Capture_Cursor.BackColor = If(enabled,
            Color.FromArgb(46, 110, 40), Color.FromArgb(90, 90, 90))
    End Sub

    Private Sub Capture_Cursor_Click(sender As Object, e As EventArgs) Handles Capture_Cursor.Click
        Try
            Dim enabled As Boolean = Not ReadCaptureCursorConfig()
            WriteCaptureCursorConfig(enabled)
            UpdateCaptureCursorLabel(enabled)
        Catch ex As Exception
            Debug.WriteLine("Capture_Cursor_Click Error: " & ex.Message)
        End Try
    End Sub
