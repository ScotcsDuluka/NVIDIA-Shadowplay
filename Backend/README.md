...
`deploy/install.ps1` builds the GFE product tree:

```
C:\Program Files\NVIDIA Corporation\NVIDIA GeForce Experience\
├── osc\          ← frontend (static, served by this backend)
├── Backend\      ← this folder + node_modules
└── Share.exe     ← Phase 2 coordinator (spawns Desktop + Hook overlays)
```

Autostart: `HKCU Run → Share.exe` (coordinator). See `deploy/` for the
scripts, `docs/osc/16-real-host-portable.md` for the full provisioning
background.
