...
Partial Public Class CaptureEngine
    ...
    Private Function BuildFFmpegArguments(outputFile As String) As String
        ...
        ' Temporary video is an intermediate artifact consumed immediately by
        ' the mux process. +faststart would force FFmpeg to do a second pass over
        ' this file at stop time just to move the moov atom. That work has zero
        ' value before muxing and makes shutdown slower. Keep faststart only for
        ' the final directly-recorded video output.
        Dim isTempVideo As Boolean = outputFile.IndexOf(".video.tmp.", StringComparison.OrdinalIgnoreCase) >= 0
        Dim ext As String = Path.GetExtension(outputFile).ToLowerInvariant()
        If Not isTempVideo AndAlso (ext = ".mp4" OrElse ext = ".mov" OrElse ext = ".m4v") Then
    ...
    '''
    ''' Duration:
    '''   -t <videoDuration> = trim output to exact video duration
    ''' </summary>
