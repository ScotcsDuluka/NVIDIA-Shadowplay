  ...
  ! function(e, r) {
    ...
    function w(e) {
      function l(t) {
        ...
        n.enter().append("rect").attr("class", "overlay").attr("pointer-events", "all").attr("cursor", N.overlay)
          .merge(n).each(function() {
            var e = g(this).extent;
            i.select(this).attr("x", e[0][0]).attr("y", e[0][1]).attr("width", e[1][0] - e[0][0]).attr("height", e[
              1][1] - e[0][1])
          }), t.selectAll(".selection").data([h("selection")]).enter().append("rect").attr("class", "selection").attr(
            "cursor", N.selection).attr("fill", "#777").attr("fill-opacity", .3).attr("stroke", "#fff").attr(
        ...
        r.exit().remove(), r.enter().append("rect").attr("class", function(e) {
          ...
          return N[e.type]
        }), t.each(b).attr("fill", "none").attr("pointer-events", "all").on("mousedown.brush", $).filter(j).on(
          "touchstart.brush", $).on("touchmove.brush", w).on("touchend.brush touchcancel.brush", k).style(
      ...
      function $() {
        function t() {
          var e = ne(j);
          !te || P || U || (Math.abs(e[0] - ie[0]) > Math.abs(e[1] - ie[1]) ? U = !0 : P = !0), ie = e, k = !0, u(),
            r()
        }
        ...
        function r() {
          ...
          switch (Z = ie[0] - re[0], ee = ie[1] - re[1], z) {
            case C:
            case T:
              q && (Z = Math.max(K - d, Math.min(Q - v, Z)), h = d + Z, _ = v + Z), G && (ee = Math.max(X - p, Math
                .min(J - $, ee)), m = p + ee, w = $ + ee);
              break;
            case x:
              q < 0 ? (Z = Math.max(K - d, Math.min(Q - d, Z)), h = d + Z, _ = v) : q > 0 && (Z = Math.max(K - v, Math
                .min(Q - v, Z)), h = d, _ = v + Z), G < 0 ? (ee = Math.max(X - p, Math.min(J - p, ee)), m = p + ee,
                w = $) : G > 0 && (ee = Math.max(X - $, Math.min(J - $, ee)), m = p, w = $ + ee);
              break;
            case S:
              q && (h = Math.max(K, Math.min(Q, d - Z * q)), _ = Math.max(K, Math.min(Q, v + Z * q))), G && (m = Math
                .max(X, Math.min(J, p - ee * G)), w = Math.max(X, Math.min(J, $ + ee * G)))
          }
          _ < h && (q *= -1, e = d, d = v, v = e, e = h, h = _, _ = e, B in I && se.attr("cursor", N[B = I[B]])), w <
            m && (G *= -1, e = p, p = $, $ = e, e = m, m = w, w = e, B in O && se.attr("cursor", N[B = O[B]])), V
            .selection && (Y = V.selection), P && (h = Y[0][0], _ = Y[1][0]), U && (m = Y[0][1], w = Y[1][1]), Y[0][
            0] === h && Y[0][1] === m && Y[1][0] === _ && Y[1][1] === w || (V.selection = [
              [h, m],
              [_, w]
            ], b.call(j), oe.brush())
        }
        ...
        function a() {
          ...
          } else n.dragEnable(i.event.view, k), ce.on("keydown.brush keyup.brush mousemove.brush mouseup.brush",
          null);
          ae.attr("pointer-events", "all"), se.attr("cursor", N.overlay), V.selection && (Y = V.selection), y(Y) && (V
            .selection = null, b.call(j)), oe.end()
        }
        ...
        function s() {
          switch (i.event.keyCode) {
            case 16:
              ...
              break;
            case 18:
              z === x && (q && (v = _ - Z * q, d = h + Z * q), G && ($ = w - ee * G, p = m + ee * G), z = S, r());
              break;
            case 32:
              z !== x && z !== S || (q < 0 ? v = _ - Z : q > 0 && (d = h - Z), G < 0 ? $ = w - ee : G > 0 && (p = m -
                ee), z = C, se.attr("cursor", N.selection), r());
              break;
            default:
              return
          }
        ...
        function l() {
          switch (i.event.keyCode) {
            ...
            case 18:
              ...
              break;
            case 32:
              z === C && (i.event.altKey ? (q && (v = _ - Z * q, d = h + Z * q), G && ($ = w - ee * G, p = m + ee *
                G), z = S) : (q < 0 ? v = _ : q > 0 && (d = h), G < 0 ? $ = w : G > 0 && (p = m), z = x), se.attr(
                "cursor", N[B]), r());
              break;
            default:
              return
        ...
        if ((!L || i.event.touches) && F.apply(this, arguments)) {
          var d, h, p, m, v, _, $, w, k, P, U, j = this,
            B = i.event.target.__data__.type,
            z = "selection" === (H && i.event.metaKey ? B = "overlay" : B) ? T : H && i.event.altKey ? S : x,
            q = e === M ? null : D[B],
            G = e === A ? null : R[B],
            V = g(j),
            W = V.extent,
      ...
      return l.move = function(t, n) {
        ...
        emit: function(t) {
          i.customEvent(new s(l, t, e.output(this.state.selection)), B.apply, B, [t, this.that, this.args])
        }
      }, l.extent = function(e) {
