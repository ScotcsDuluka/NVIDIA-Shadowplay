      ...
      var h = c.SHA3 = o.extend({
        ...
        _doProcessBlock: function(e, t) {
          for (var n = this._state, r = this.blockSize / 2, i = 0; i < r; i++) {
            var o = e[t + 2 * i],
              a = e[t + 2 * i + 1];
            o = 16711935 & (o << 8 | o >>> 24) | 4278255360 & (o << 24 | o >>> 8), a = 16711935 & (
              a << 8 | a >>> 24) | 4278255360 & (a << 24 | a >>> 8);
            var s = n[i];
            s.high ^= a, s.low ^= o;
          }
          for (var c = 0; c < 24; c++) {
            ...
            for (var h = 0; h < 5; h++)
              for (var y = f[(h + 4) % 5], b = f[(h + 1) % 5], E = b.high, _ = b.low, p = y.high ^ (
                  E << 1 | _ >>> 31), m = y.low ^ (_ << 1 | E >>> 31), v = 0; v < 5; v++) {
                var s = n[h + 5 * v];
                s.high ^= p, s.low ^= m;
              }
            for (var $ = 1; $ < 25; $++) {
        ...
        _doFinalize: function() {
          var e = this._data,
            ...
            o = 32 * this.blockSize;
          n[r >>> 5] |= 1 << 24 - r % 32, n[(t.ceil((r + 1) / o) * o >>> 5) - 1] |= 128, e
            .sigBytes = 4 * n.length, this._process();
          for (var a = this._state, s = this.cfg.outputLength / 8, c = s / 8, u = [], l = 0; l <
            c; l++) {
            var d = a[l],
              f = d.high,
              h = d.low;
            f = 16711935 & (f << 8 | f >>> 24) | 4278255360 & (f << 24 | f >>> 8), h = 16711935 & (
              h << 8 | h >>> 24) | 4278255360 & (h << 24 | h >>> 8), u.push(h), u.push(f);
          }
