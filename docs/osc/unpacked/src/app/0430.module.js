  ...
  ! function(e, i) {
    ...
    function E(e) {
      function c(t) {
        ...
        n.enter().append("rect").attr("class", "overlay").attr("pointer-events", "all").attr("cursor", M
          .overlay).merge(n).each(function() {
          var e = b(this).extent;
          o.select(this).attr("x", e[0][0]).attr("y", e[0][1]).attr("width", e[1][0] - e[0][0]).attr(
            "height", e[1][1] - e[0][1]);
        }), t.selectAll(".selection").data([m("selection")]).enter().append("rect").attr("class",
          "selection").attr("cursor", M.selection).attr("fill", "#777").attr("fill-opacity", .3).attr(
        ...
        i.exit().remove(), i.enter().append("rect").attr("class", function(e) {
            ...
            return M[e.type];
          }), t.each(v).attr("fill", "none").attr("pointer-events", "all").on("mousedown.brush", S).filter(
            G).on("touchstart.brush", S).on("touchmove.brush", E).on("touchend.brush touchcancel.brush", I)
      ...
      function S() {
        function t() {
          var e = ne(G);
          !te || L || U || (Math.abs(e[0] - oe[0]) > Math.abs(e[1] - oe[1]) ? U = !0 : L = !0), oe = e,
            I = !0, d(), i();
        }
        ...
        function i() {
          ...
          switch (J = oe[0] - ie[0], ee = oe[1] - ie[1], B) {
            case _:
            case k:
              Y && (J = Math.max(q - u, Math.min(Z - h, J)), m = u + J, w = h + J), $ && (ee = Math.max(X -
                g, Math.min(Q - S, ee)), p = g + ee, E = S + ee);
              break;
            case T:
              Y < 0 ? (J = Math.max(q - u, Math.min(Z - u, J)), m = u + J, w = h) : Y > 0 && (J = Math.max(
                q - h, Math.min(Z - h, J)), m = u, w = h + J), $ < 0 ? (ee = Math.max(X - g, Math.min(Q -
                g, ee)), p = g + ee, E = S) : $ > 0 && (ee = Math.max(X - S, Math.min(Q - S, ee)), p = g,
                E = S + ee);
              break;
            case C:
              Y && (m = Math.max(q, Math.min(Z, u - J * Y)), w = Math.max(q, Math.min(Z, h + J * Y))), $ &&
                (p = Math.max(X, Math.min(Q, g - ee * $)), E = Math.max(X, Math.min(Q, S + ee * $)));
          }
          w < m && (Y *= -1, e = u, u = h, h = e, e = m, m = w, w = e, H in R && le.attr("cursor", M[H = R[
              H]])), E < p && ($ *= -1, e = g, g = S, S = e, e = p, p = E, E = e, H in P && le.attr(
              "cursor", M[H = P[H]])), W.selection && (K = W.selection), L && (m = K[0][0], w = K[1][0]),
            U && (p = K[0][1], E = K[1][1]), K[0][0] === m && K[0][1] === p && K[1][0] === w && K[1][1] ===
            E || (W.selection = [
              [m, p],
              [w, E]
            ], v.call(G), re.brush());
        ...
        function a() {
          ...
          } else n.dragEnable(o.event.view, I), se.on(
            "keydown.brush keyup.brush mousemove.brush mouseup.brush", null);
          ae.attr("pointer-events", "all"), le.attr("cursor", M.overlay), W.selection && (K = W.selection),
            x(K) && (W.selection = null, v.call(G)), re.end();
        }
        ...
        function l() {
          switch (o.event.keyCode) {
            case 16:
              ...
              break;
            case 18:
              B === T && (Y && (h = w - J * Y, u = m + J * Y), $ && (S = E - ee * $, g = p + ee * $), B = C,
                i());
              break;
            case 32:
              B !== T && B !== C || (Y < 0 ? h = w - J : Y > 0 && (u = m - J), $ < 0 ? S = E - ee : $ > 0 &&
                (g = p - ee), B = _, le.attr("cursor", M.selection), i());
              break;
            default:
              return;
          }
        ...
        function c() {
          switch (o.event.keyCode) {
            ...
            case 18:
              ...
              break;
            case 32:
              B === _ && (o.event.altKey ? (Y && (h = w - J * Y, u = m + J * Y), $ && (S = E - ee * $, g =
                p + ee * $), B = C) : (Y < 0 ? h = w : Y > 0 && (u = m), $ < 0 ? S = E : $ > 0 && (g =
                p), B = T), le.attr("cursor", M[H]), i());
              break;
            default:
              return;
          }
        ...
        if ((!F || o.event.touches) && z.apply(this, arguments)) {
          var u,
            ...
            B = "selection" === (V && o.event.metaKey ? H = "overlay" : H) ? k : V && o.event.altKey ? C :
            T,
            Y = e === A ? null : D[H],
            $ = e === O ? null : N[H],
            W = b(G),
      ...
      return c.move = function(t, n) {
        ...
        emit: function(t) {
          o.customEvent(new l(c, t, e.output(this.state.selection)), H.apply, H, [t, this.that, this
            .args
          ]);
        }
      }, c.extent = function(e) {
