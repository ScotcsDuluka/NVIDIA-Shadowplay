...
function(module, exports, require) {
  "use strict";
  require(40) /* app/40 — nvCameraService (service) */, require(12) /* app/12 — oscDisplayService (service) */, require(5) /* app/5 — hoverFocus (directive) */, angular.module("main").controller("NvCameraMenuController", ["$scope",
    "$state", "$stateParams", "$log", "$document", "$timeout", "$q", "$filter", "$window",
    "eventAggregator", "nvCameraService", "oscDisplayService", "shadowPlayService", "cefService",
    "settingsService", "galleryService", "oscNotificationService", "telemetryService", "hotkeyService",
    "KEYBOARD_EVENTS", "NVCAMERA_EVENTS", "NOTIFIER_SELECTIONS", "TELEMETRY_OSC_EVENT_NAMES",
    "TELEMETRY_OSC_TOGGLE_STATE", "OSC_KEYBOARD", "GAMEPAD_EVENTS", "COMMON_EVENTS", "NVCAMERA_STATUS",
    "HOTKEY_EVENTS", "SHADOWPLAY_EVENTS", "NGX_NOTIFICATIONS", "TELEMETRY_OSC_AVAILABLE_STATUS",
    "TELEMETRY_OSC_SCREEN_STATE", "NVCAMERA_MODE",
    function(e, t, n, i, o, r, a, l, s, d, c, u, f, m, g, p, h, b, x, v, y, w, S, E, k, T, C, O, A, I, M,
      R, P, D) {
      function N() {
      ...
      function L(e) {
        var t;
        return 0 === e.length ? void(we.Resolution.enabled = !1) : (we.Resolution.enabled = !0, t = e
          .length - 1, we.Resolution.resolutions = e, we.Resolution.value = 0, we.Resolution.range.min =
          0, we.Resolution.range.max = t, we.currentResolution = we.Resolution.resolutions[we.Resolution
            .value], d.off(y.HIGHRES_RESOLUTIONS, L), void Se.info("fillHighResResolutionSlider: ", we
            .Resolution));
      }
      ...
      function F() {
        ...
        for (var i = 0; i <= we.Resolution.range.max; i++) {
          var o = {};
          i > 0 ? (o.multiplier = 2 * e[i - 1].multiplier, we.Resolution.enabled = !0) : o.multiplier =
            i + 2, o.w = Fe.width * o.multiplier, o.h = Fe.height * o.multiplier, e.push(o);
        }
        we.Resolution.resolutions = e, we.currentResolution = we.Resolution.resolutions[we.Resolution
          .value];
      }

      function U(e) {
        !_.isUndefined(e) && _.isNumber(e.w) && _.isNumber(e.h) && ("stereoRegular" === we.captureType
          .id && (e.w = 2 * e.w), Fe.width = e.w, Fe.height = e.h, Se.info(
            "Game resolution (W x H) = (" + e.w + " x " + e.h + ")."), we.Resolution.resolutions = [e],
          we.Resolution.value = 0, we.Resolution.range = {
            min: 0,
            max: 0
          }, we.currentResolution = we.Resolution.resolutions[we.Resolution.value], we.Resolution
          .enabled = !1, d.off(y.SCREENSHOT_RESOLUTION, U), Se.info("fillScreenshotResolutionSlider: ",
            we.Resolution));
      }
      ...
      function z(e) {
        ...
        var i = e.minX,
          ...
          a = (o - i) / 8;
        for (Se.info("step: ", a), we.Resolution.resolutions = [], n = 0; n <= t; n++) we.Resolution
          .resolutions[n] = {
            w: i + a * n,
            h: (i + a * n) / r
          };
        we.Resolution.range.min = 0, we.Resolution.range.max = t, we.Resolution.value = 0, we
          .currentResolution = we.Resolution.resolutions[we.Resolution.value], d.off(y
            .PANORAMA_RESOLUTION_RANGE, z), Se.info("fillPanoramaResolutionSlider: ", we.Resolution);
      }
      ...
      function H() {
        var e = 0,
          t = 1;
        e = we.panes.Camera.items.Roll.value - t, e >= we.panes.Camera.items.Roll.range.min && (we
          .cameraAdjustRoll(e), we.panes.Camera.items.Roll.value = e);
      }

      function B() {
        var e = 0,
          t = 1;
        e = we.panes.Camera.items.Roll.value + t, e <= we.panes.Camera.items.Roll.range.max && (we
          .cameraAdjustRoll(e), we.panes.Camera.items.Roll.value = e);
      }
      ...
      function j() {
        ...
        we.isAnselLiteMode ? b.startTimer(S.OSC_ANSEL_LITE_SCREENSHOT_TAKEN) : b.startTimer(S
            .OSC_ANSEL_SCREENSHOT_COMPLETED), c.sendStartTimerTelemetry(S.OSC_ANSEL_SCREENSHOT_CANCELLED),
          Oe.screenshotType = e, Oe.superResolutionFactor = 1, Oe.filterID = "0", Oe.stackedFilters = c
          .getStackedFilterDisplayNames(we.filterSlot), "highRes" === we.captureType.id && (Oe
            .superResolutionFactor = we.currentResolution.multiplier), Ie.screenshotType = Oe
          .screenshotType, Ie.screenshotResolution = Oe.screenshotResolution, Ie.superResolutionFactor =
          Oe.superResolutionFactor, Ie.hdrMode = Oe.hdrMode, we.isAnselLiteMode ? Ie.mode = D.anselLite :
          Ie.mode = D.gamePhoto, we.panningUsed ? Ie.panningUsed = R.yes : Ie.panningUsed = R.no,
          "highRes" === we.captureType.id && (Ie.method = we.HighResEnhanceMenu.selectedOption.id), c
          .sendScreenshotParams(Ie), c.sendScreenshotTelemetry(S.OSC_ANSEL_SCREENSHOT_STARTED), d.off(y
            .SCREENSHOT_CAPTURE_STARTED, j);
      }
      ...
      function X(e) {
        ...
        Ae.screenshotType = Oe.screenshotType, Ae.screenshotResolution = Oe.screenshotResolution, Ae
          .superResolutionFactor = Oe.superResolutionFactor, Ae.stackedFilters = Oe.stackedFilters, Ae
          .hdrMode = Oe.hdrMode, Ae.grid = Oe.grid, Oe.method = "None", we.panningUsed ? Ae.panningUsed =
          R.yes : Ae.panningUsed = R.no, "highRes" === we.captureType.id && (Ae.method = we
            .HighResEnhanceMenu.selectedOption.id, Oe.method = we.HighResEnhanceMenu.selectedOption.id), m
          ...
          .sendFilterTelemetry(Oe.stackedFilters, 0, "nvcamera"), h.show(w
            .PHOTOGRAPHIC_SCREENSHOT_SAVED_TO_GALLERY, t), c.sendFilterControlTelemetry("nvcamera", we
            .filterSlot), d.off(y.SCREENSHOT_CAPTURE_DONE, X);
      }
      ...
      function Q(e) {
        e && (e.ngxFailureReason ? e.ngxFailureReason != O.SCREENSHOT_TIMEOUT_FAILURE && e
            .ngxFailureReason != O.SCREENSHOT_GENERIC_FAILURE || (we.errorDialogParam.error =
            ...
            .FAILED_TO_SAVE_SHOT_NO_SPACE_LEFT ? (we.errorDialogParam.error =
              ...
              "l10n.anselScreenshotFail.lowDiskSpace.message", e.data.hasOwnProperty("diskspaceReq") && (
                we.errorDialogParam.arg1 = e.data.diskspaceReq), e.data.hasOwnProperty("dirpath") && (we
                .errorDialogParam.arg2 = e.data.dirpath)) : e.errorId == O.PERMISSION_DENIED ? (we
              .errorDialogParam.error = "l10n.anselScreenshotFail.permissionDenied.title", we
              .errorDialogParam.details = "l10n.anselScreenshotFail.permissionDenied.message", e.data
              .hasOwnProperty("dirpath") && (we.errorDialogParam.arg1 = e.data.dirpath)) : (we
              .errorDialogParam.error = "l10n.anselScreenshotFail.genericFailure.title", we
              .errorDialogParam.details = "l10n.anselScreenshotFail.genericFailure.message")), we
          .captureInProgress = !1, we.errorDialogParam.isCustomError = !0, d.off(y
            .SCREENSHOT_CAPTURE_FAILED, Q), d.off(y.SCREENSHOT_CAPTURE_INPROGRESS, K), d.off(y
            .SCREENSHOT_CAPTURE_DONE, X), d.off(y.SCREENSHOT_CAPTURE_FINISHED, q), d.off(y
      ...
      function J(e) {
        ...
        else if (e.notification === M.AI_SUPER_RES_PROGRESS) void 0 != e.progress && (t = e.progress, t <
          100 && we.triggerCaptureProgressEvent("progress", t));
      ...
      function ee() {
        if (Ce.length > 0) {
          ...
          "png" !== t && (e = Ce[0].slice(0, e.length - 4), e = e.concat(".png"), Ce.push(e)), f
            .captureNGXShot(J, we.currentResolution.multiplier, e);
        }
      }
      ...
      function ne() {
        we.filterSlot = c.modsSlots[c.ANSEL_MOD_SLOT], c.applySlot(we.filterSlot), d.off(y
          .MODS_FILTERS_LOADED, ne);
      }

      function ie(e) {
        we.panes.Camera.items.Roll.range = e[0], we.panes.Camera.items.Roll.value = 0, d.off(y
          .CAMERA_ROLL_RANGE, ie);
      }

      function oe(e) {
        we.panes.Camera.items.FOV.range = e[0], (we.panes.Camera.items.FOV.value < e[0].min || we.panes
          .Camera.items.FOV.value > e[0].max) && (we.panes.Camera.items.FOV.value = e[0].min), d.off(y
          .CAMERA_FOV_RANGE, oe);
      }

      function re(e) {
        we.panes.Camera.items.FOV.default = e, we.panes.Camera.items.FOV.value = e, Oe.fov = Math.round(
          e), d.off(y.CAMERA_FOV_VALUE, re);
      }
      ...
      function le(e) {
        if (Se.info("addUIElement called"), _.isUndefined(e) || _.isNull(e) || _.isUndefined(e
            .controlType) || _.isNull(e.controlType) || _.isUndefined(e.controlId) || _.isNull(e
            .controlId)) {
          var t = {};
        ...
        if (we.panes.GameEngine.show || (we.panes.GameEngine.show = !0), "slider" === e.controlType) {
          if (_.isUndefined(e.minValue) || _.isNull(e.minValue) || isNaN(e.minValue) || _.isUndefined(e
              .maxValue) || _.isNull(e.maxValue) || isNaN(e.maxValue) || _.isUndefined(e.defaultValue) ||
            _.isNull(e.defaultValue) || isNaN(e.defaultValue) || _.isUndefined(e.stepSize) || _.isNull(e
              .stepSize) || isNaN(e.stepSize)) {
            var t = {};
            throw t.data = e, t.message =
              "addUIElement called with invalid data for control of type slider", t;
          }
          Se.info("Adding game engine slider '" + e.displayName + "'"), we.panes.GameEngine.items.push({
            ...
            used: !1,
            onChange: function(t) {
              we.panes.GameEngine.savedConfigs[t.id] = t.value, c.sendSandboxAttribute(t.id, e
                .controlType, t.value), this.used = ge(this);
            },
            onReset: function() {
          ...
          if (_.isUndefined(e.currentValue) || _.isNull(e.currentValue)) {
            var t = {};
            throw t.data = e, t.message =
              "addUIElement called with invalid data for control of type boolean!", t;
          }
          Se.info("Adding game engine boolean '" + e.displayName + "'"), we.panes.GameEngine.items.push({
            ...
            used: !1,
            onChange: function(t) {
              we.panes.GameEngine.savedConfigs[t.id] = t.set, c.sendSandboxAttribute(t.id, e
                .controlType, t.set), this.used = ge(this);
            },
            onKeyDown: function(e) {
              (this.set && e.keyCode === k.LEFT_ARROW || !this.set && e.keyCode === k
              .RIGHT_ARROW) && (this.set = !this.set, this.onChange(this)), c
        ...
        } else if ("button" === e.controlType) Se.info("Adding sandbox button '" + e.displayName + "'"),
          we.panes.Sandbox.items.push({
            ...
            used: !1,
            onClick: function() {
              console.info("button " + this.buttonText + " clicked"), c.sendSandboxAttribute(this.id,
                e.controlType), this.used = ge(this);
            }
          });
          ...
          _.each(e.listItems, function(e, t) {
            ...
            selectItem: function(t) {
              Se.info("item selected from list:", t), c.sendSandboxAttribute(this.id, e.controlType,
                t.id), this.used = ge(this);
            },
            title: e.displayName,
            selectedItem: "",
            items: i
        ...
        } else if ("label" === e.controlType) Se.info("Adding sandbox label '" + e.displayName + "'"), we
          .panes.Sandbox.items.push({
            id: e.controlId,
            title: e.displayName,
            type: "sidebar-label",
            enabled: "data.visible"
          ...
          switch (e.allowedType) {
            case 0:
              o = "number", r = -2147483648, a = 2147483647, l = 1, d = 0, u = function() {
                this.value = parseInt(this.value.replace(/[^\-0-9]/g, "")), this.value.lastIdexOf("-") >
                  0 && (this.value = parseInt(this.value)), this.used = ge(this), Se.debug(
                    "Edit text submitted: ", this.value), c.sendSandboxAttribute(this.id, e.controlType,
                    this.value);
              };
              break;
            case 1:
              o = "number", r = 0, a = 4294967295, l = 1, d = 0, u = function() {
                this.value = this.value.replace(/[^0-9]/g, "").trim(), Se.debug("Edit text submitted: ",
                    this.value), c.sendSandboxAttribute(this.id, e.controlType, this.value), this.used =
                  ge(this);
              };
              break;
            case 2:
              l = .001, o = "number", d = 0, u = function() {
                this.value = this.value.replace(/[^\-0-9.,]/g, ""), this.value.lastIndexOf("-") > 0 && (
                    this.value = parseFloat(this.value)), (this.value.match(/[,.]/g) || []).length >
                  1 && (this.value = parseFloat(this.value)), Se.debug("Edit text submitted: ", this
                    .value), c.sendSandboxAttribute(this.id, e.controlType, this.value), this.used = ge(
                    this);
              };
              break;
            case 3:
      ...
      function ue(e) {
        Se.info("removeAllGameSettingControls called. Clearing the GameSetting controls."), we.panes
          .GameEngine.items.splice(0);
      }
      ...
      function me(e) {
        if (e && e.captureTypes) {
          we.CaptureMenu.items.forEach(function(t) {
            t.disabled = e.captureTypes.indexOf(t.id) === -1, "highRes" === t.id && we
              .HighResEnhanceMenu.items.forEach(function(e) {
                t.disabled && (e.disabled = !0), "ngx" === e.id && (e.disabled = !we
                  .isRTXUpResAvailable, t.disabled = t.disabled && e.disabled);
              });
          });
          for (var t = 0; t < we.HighResEnhanceMenu.items.length; t++)
            if (!we.HighResEnhanceMenu.items[t].disabled) {
              we.HighResEnhanceMenu.isAvailable = !0, we.HighResEnhanceMenu.selectedOption = we
                .HighResEnhanceMenu.items[t];
              break;
            }
      ...
      function ge(e) {
        return !(!e || _.isUndefined(e) || _.isNull(e)) && (e.used === !1 && ("None" === Oe
          .gameEngineControls ? Oe.gameEngineControls = e.title : Oe.gameEngineControls += ", " + e
          .title), !0);
      }

      function pe() {
        return we.captureInProgress ? we.cancelCapture() : we.MousePanningActive && we.isAnselLiteMode ?
          ye(!0) : we.close();
      }
      ...
      function xe() {
        Se.info("NvCameraMenuController::onGameAppExit invoked. UIRunning = " + c.getUIRunning()), u
          .closeOSC();
      }
      ...
      function ve(e, t, n, i) {
        if (e) {
          var o = !1;
          e.keyCode === t ? (m.oscSendWinKBMessage(k.TAB, 0), o = !0) : e.keyCode === n && (m
            .oscSendWinKBMessage(k.TAB, k.SHIFT), o = !0), o && i && e.stopImmediatePropagation();
        }
      }
      ...
      function ye(e) {
        ...
        var t = ["mouse"];
        we.MousePanningActive ? (f.setInputRedirection(!1, t, "spuirleak"), we.MousePanningActive = !1,
          be()) : e || m.isInDesktopMode().then(function(e) {
          e || (we.MousePanningActive = !0, we.panningUsed = !0, f.setInputRedirection(!0, t,
            "spuirleak").then(function() {}));
        });
      }
      var we = this,
      ...
      we.captureInProgress = !1, we.cancelInProgress = !1, we.MousePanningActive = !1, we
        .percentComplete = 0, we.redirectEnabled = !1, we.styleTransferEnabled = !1, we
        .showFolderBrowser = !1;
      var Me = null,
        Re = null,
        Pe = null;
        ...
        .panningUsed = !1, we.panningWithKB = !1, we.panningWithMouse = !1, we.FilterStack = {
          ...
          onKeyDown: function(e, t) {
            e.keyCode === k.DELETE && this.onStackItemDelete(), e.keyCode !== k.ENTER && e.keyCode !== k
              .RIGHT_ARROW || (this.onStackItemClick(t), we.FilterStack.activationPromise && we
                .FilterStack.activationPromise.then(function() {
          ...
          onStackItemClick: function(e) {
            we.filterSlot.activeFilter !== e ? we.FilterStack.activationPromise = a.when(!0) : we
              .FilterStack.activationPromise = null, c.modsHelper.activateFilter(we.filterSlot, e);
          },
          ...
          onStackItemDelete: function() {
            c.modsHelper.removeActiveItem(we.filterSlot).then(function() {
              c.modsHelper.trashDisabled(we.filterSlot) && o[0].getElementById("filter-dropdown")
                .focus();
            });
          },
          getLen: function() {
            return we.filterSlot && we.filterSlot.activeFilterStack ? we.filterSlot.activeFilterStack
              .length : 0;
          },
          ...
          onSelectMenuKeyDown: function(e, t) {
            e.keyCode === k.ENTER && we.stackThisFilter(t), e.keyCode === k.RIGHT_ARROW && (m
              .oscSendWinKBMessage(k.ENTER, 0), e.stopImmediatePropagation());
          },
          onSelectKeyDown: function(e) {
            e.keyCode == k.DOWN_ARROW ? (we.onKeyDown(e), e.stopImmediatePropagation()) : e.keyCode == k
              .UP_ARROW ? (we.onKeyDown(e), e.stopImmediatePropagation()) : ve(e, k.RIGHT_ARROW, void 0,
                !0);
          },
          ...
          supported: !0,
          onKeyDown: function(e) {
            (this.isOn && e.keyCode === k.LEFT_ARROW || !this.isOn && e.keyCode === k.RIGHT_ARROW) && (
              this.isOn = !this.isOn, this.onChange()), c.setNavigationInputDevice(["keyboard"], [!0]);
          },
          onChange: function() {
            Oe.hdrMode = this.isOn ? E.on : E.off;
          },
          isExrModeSupported: function() {
            return "highRes" === we.captureType.id && "ngx" === we.HighResEnhanceMenu.selectedOption
              .id ? (we.exrMode.isOn = !1, !1) : we.exrMode.supported;
          }
        }, we.gridOfThirds = {
          isOn: !1,
          onKeyDown: function(e) {
          ...
          onChange: function() {
            this.isOn && we.isAnselLiteMode && (this.gridLines.height = o[0].getElementById(
              "nvcameraContainer").clientHeight, this.gridLines.width = o[0].getElementById(
              "nvcameraContainer").clientWidth, this.gridLines.line_horizontal_1 = {
              x1: 0,
          ...
          var t = "";
          return "highRes" === we.captureType.id && (t = we.Resolution.resolutions[e].multiplier +
            "X - "), Oe.screenshotResolution = we.Resolution.resolutions[e].w + " x " + we.Resolution
            .resolutions[e].h, t + we.Resolution.resolutions[e].w + " x " + we.Resolution.resolutions[e]
            .h;
          ...
          return Se.info("Current filter application started"), we.filterSlot.selectedFilter = e, c
            .modsHelper.addCurrentFilter(we.filterSlot).then(function(e) {
          ...
          if (!e) return we.styleTransferEnabled = !1, we.panes.StyleTF.items.Model.enabled = !1, we.panes
            .StyleTF.items.File.enabled = !1, void(we.panes.StyleTF.items.Model.items = []);
          we.styleTransferEnabled = !0, we.panes.StyleTF.items.Model.enabled = !0, we.panes.StyleTF.items
            .File.enabled = !0;
          var t = [{
            id: 0,
            title: "Low"
          ...
          var t = e;
          return t && t.lastIndexOf("\\") > 0 && (t = t.slice(t.lastIndexOf("\\") + 1)), we.panes.StyleTF
            .items.File.fileName = t, Se.info("path is: ", e), we.showFolderBrowser = !1, a.when(!0);
        }, we.selectStyleTransferPath = function() {
          var e = we.styleTransferPath || "",
            t = we.styleTransferPath || "";
          t.length > 0 && e.lastIndexOf("\\") > 0 ? (t = t.slice(0, t.lastIndexOf("\\")), e = e.slice(e
              .lastIndexOf("\\") + 1)) : (e = "", t = "c:\\users\\irichards\\Pictures"), we
            .filePickerParams = {
      ...
      e.$on("$mdMenuClose", function(e, t) {
        ...
        if (e !== we.captureType) {
          ...
          var t = null;
          t = "highRes" === e.id ? "highres" : "sphericalPanorama" === e.id ||
            "stereoSphericalPanorama" === e.id ? "panorama" : "screenshot", null !== t ? ("highres" ===
              t && "ngx" === we.HighResEnhanceMenu.selectedOption.id ? (F(), we.captureType = e) : c
              .getCaptureTypeResolutions(t).then(function(n) {
                n === !0 && ("highres" === t ? d.on(y.HIGHRES_RESOLUTIONS, L) : "panorama" === t ? d
                  .on(y.PANORAMA_RESOLUTION_RANGE, z) : "screenshot" === t && d.on(y
                    .SCREENSHOT_RESOLUTION, U), we.captureType = e);
              }), we.Resolution.tabIndex = "screenshot" === t ? -1 : 0) : (we.captureType = e, we
              .Resolution.value = 0, we.Resolution.enabled = !1, we.Resolution.tabIndex = -1);
        }
      }, we.resolutionChanged = function(e) {
        we.Resolution.value = e, Oe.superResolutionFactor = e, we.currentResolution = we.Resolution
          .resolutions[we.Resolution.value];
      }, we.cameraAdjustRoll = function(e) {
        var t = Math.round(e);
        t ? Oe.roll = t : Oe.roll = 0, d.on(y.CAMERA_ROLL_VALUE_SET, G), c.setCameraAdjustments(e, we
          .panes.Camera.items.FOV.value);
      }, we.cameraAdjustFOV = function(e) {
        var t = Math.round(e);
        t ? Oe.fov = t : Oe.fov = 0, d.on(y.CAMERA_FOV_VALUE_SET, V), c.setCameraAdjustments(we.panes
          .Camera.items.Roll.value, e);
        ...
        switch (we.captureType.id) {
          ...
          case "stereoSphericalPanorama":
          case "sphericalPanorama":
            c.capturePanoramaScreenshot(we.captureType.id, we.currentResolution.w, we.currentResolution
              .h, we.exrMode.isOn);
        }
        ...
        return Se.info("Cancelling current screenshot capture."), we.cancelInProgress = !0, d.off(y
            .SCREENSHOT_CAPTURE_INPROGRESS, K), "highRes" === we.captureType.id && "ngx" === we
          .HighResEnhanceMenu.selectedOption.id ? f.cancelNGXShot().then(function(e) {
            if (Ce)
              for (; Ce.length > 0;) p.removeGalleryItem(Ce.pop(), !0);
            Z();
        ...
        if (!we.isAnselLiteMode) return c.setNavigationInputDevice(["mouse"], [!0]), we
          .redirectEnabled === !0 ? a.when(!0) : m.isInDesktopMode().then(function(t) {
            return f.setInputRedirection(!0, e, t === !0 ? "windowed" : "fullscreen").then(
            function() {
              return we.redirectEnabled = !0;
            });
          });
      }, we.disableRedirect = function(e) {
        if (!we.isAnselLiteMode) return document.body.style.cursor = "auto", c.setNavigationInputDevice(
            ["mouse"], [!0]), we.redirectEnabled === !1 ? a.when(!1) : f.setInputRedirection(!1, e)
          .then(function(e) {
            return we.redirectEnabled = !1;
          });
      }, we.gamepadRedirect = function(e, t) {
        if (!we.isAnselLiteMode) return e ? we.redirectEnabled === !0 ? a.when(!0) : m.isInDesktopMode()
          .then(function(n) {
            return f.setInputRedirection(e, t, n === !0 ? "windowed" : "fullscreen").then(
          function() {
              return we.redirectEnabled = !0;
            });
          }) : we.redirectEnabled === !1 ? a.when(!1) : f.setInputRedirection(!1, t).then(function(
          e) {
            return we.redirectEnabled = !1;
          });
        ...
        return Se.info("Initialize Ansel UI"), f.appInFocusTitle().then(function(e) {
          Oe.gameName = e, m.isInDesktopMode().then(function(e) {
              Ne = !e, Ne && (Me = x.createHotKey(f.HotkeyShortcuts.NVCAMERAUI, A.NVCAMERAUI),
                Re = x.createHotKey(f.HotkeyShortcuts.SCREENSHOT, A.SCREENSHOT), null != Me &&
                Me.startHotkeyDetection(), null != Re && Re.startHotkeyDetection());
            }), Pe = x.createHotKey("anselHideUIHK", A.ANSEL_HIDEUI), null != Pe && Pe
            .startHotkeyDetection();
        }), we.CaptureMenu.onMenuItemClick = we.selectCapture, we.filterSlot = void 0, c.setUIRunning(
          !0, !1, !we.isAnselLiteMode), we.gamepadRedirect(!0, ["gamepad"]), g.getLanguage().then(
          function(e) {
            return c.setLanguage(e);
          }).then(function() {
          d.on(y.CAPTURE_TYPES, me), d.on(y.MODS_FILTERS_LOADED, ne), d.on(y
              .MODS_FILTER_SETTINGS_READY, te), d.on(y.SCREENSHOT_RESOLUTION, U), d.on(T
              .LEFT_BUMPER, H), d.on(T.RIGHT_BUMPER, B), d.on(T.X_BUTTON, Y), d.on(T.Y_BUTTON, $), d
            .on(T.NAVIGATION, W), d.on(C.ELEMENT_FOCUSSED, he), d.on(I.GAME_EXITED, xe), d.on(A
              .ANSEL_HIDEUI, we.hideUIFunction), we.isAnselLiteMode || (d.on(y.CAMERA_ROLL_RANGE,
              ie), d.on(y.CAMERA_FOV_RANGE, oe), d.on(y.CAMERA_FOV_VALUE, re), d.on(y
              .ADD_UI_ELEMENT, le), d.on(y.REMOVE_UI_ELEMENT, de), d.on(y
              .GET_UI_ELEMENT_VISIBILITY, fe), d.on(y.SET_UI_ELEMENT_VISIBILITY, ce), d.on(y
              .REMOVE_ALL_GAME_SETTINGS, ue), d.on(y.CAMERA_ROLL_VALUE_UPDATE, ae));
          var e = [];
          return e.push(c.getSupportedCaptureTypes()), e.push(c.getSupportedFilterTypes()), e.push(c
            ...
            .getCameraAdjustmentsRange()), e.push(c.getCameraAdjustments())), a.all(e).then(
            function(e) {
              Se.info("getNvCameraInfo Response: ", e), we.enableEscapeEvent(!0), c.sendUIReady(),
                c.setReadyForGameEngine(), c.sendStartTelemetry(), we.isAnselLiteMode ? c
                .sendEndTimerTelemetry(S.OSC_MENU_LAUNCH, "nvcameralite") : c
                .sendEndTimerTelemetry(S.OSC_MENU_LAUNCH, "nvcamera");
        ...
        if (!Ue) return we.destroy().then(function() {
          c.sendEndTelemetry(we.panningUsed, we.panningWithKB, we.panningWithMouse), _
            .isUndefined(n.lastState) || "base" === n.lastState || "nvcamera" === n.lastState ? u
        ...
        "topMost" === t ? (we.topMostInFocus = !0, we.bottomMostInFocus = !1) : "bottomMost" === t ? (we
          .bottomMostInFocus = !0, we.topMostInFocus = !1) : (we.bottomMostInFocus = !1, we
          .topMostInFocus = !1);
      }, we.onBlur = function(e) {
        var t = angular.element(e.target).attr("data-item-position");
        "topMost" === t ? we.topMostInFocus = !1 : "bottomMost" === t && (we.bottomMostInFocus = !1);
      }, we.HandleMouseDown = function(e, t) {
        we.isAnselLiteMode || we.MousePanningActive || 0 === t.button && (document.body.style.cursor =
          "none", we.MousePanningActive = !0, we.enableRedirect(["mouse"]));
        ...
        if (we.MousePanningActive && we.isAnselLiteMode) e.preventDefault(), e.stopPropagation(), we
          ...
          case k.UP_ARROW:
            ...
            break;
          case k.TAB:
            (e.shiftKey && we.topMostInFocus || !e.shiftKey && we.bottomMostInFocus) && e
              .preventDefault();
        }
        c.setNavigationInputDevice(["keyboard"], [!0]);
      };
      ...
      we.destroy = function() {
        return Se.info("Destroying the controller"), Ue = !0, we.enableEscapeEvent(!1), a.all([we
          ...
          .gamepadRedirect(!1, ["gamepad"]), null != Me ? Me.stopHotKeyDetection() : a.when(!0),
          null != Re ? Re.stopHotKeyDetection() : a.when(!0), null != Pe ? Pe
        .stopHotKeyDetection() : a.when(!0)
        ]).then(function(e) {
          ye(!0), c.applySlot(void 0), c.setGridOfThirdsStatus(!1), c.shutdownAnselUI(), d.off(y
              .CAPTURE_TYPES, me), d.off(y.ADD_UI_ELEMENT, le), d.off(y.REMOVE_UI_ELEMENT, de), d
            .off(y.GET_UI_ELEMENT_VISIBILITY, fe), d.off(y.SET_UI_ELEMENT_VISIBILITY, ce), d.off(y
              .REMOVE_ALL_GAME_SETTINGS, ue), d.off(T.LEFT_BUMPER, H), d.off(T.RIGHT_BUMPER, B), d
            .off(T.X_BUTTON, Y), d.off(T.Y_BUTTON, $), d.off(T.NAVIGATION, W), d.off(C
              .ELEMENT_FOCUSSED, he), d.off(I.GAME_EXITED, xe), d.off(y.CAMERA_ROLL_VALUE_UPDATE,
              ae), d.off(y.MODS_FILTER_SETTINGS_READY, te), d.off(A.NVCAMERAUI, we.close), d.off(A
              .ANSEL_HIDEUI, we.hideUIFunction);
        });
      }, e.$on("$destroy", function e() {
        return Se.info("onDestroy"), Ee ? (Ue ? (we.enableEscapeEvent(!1), Ee = !1, Ue = !1) : we
