...
ShareQueryHandler::ShareQueryHandler(int http_port, const std::string& secret,
...
bool ShareQueryHandler::OnQuery(CefRefPtr<CefBrowser> browser,
  ...
  if (cmd == "QUERY_WIN_NODE_INFO") {
    ...
    // (CefQueryBridge.vb:209-213).
    std::ostringstream o;
    o << "{\"port\":" << http_port_ << ",\"secret\":\"" << secret_ << "\"}";
    response = o.str();
    log_extra = "boot handshake";
    ...
    // CEF-lane proof observer (not a production command).
    ...
    // display service (the same openOSC path the engine's Alt+Z uses) so
    // the visual evidence shows the real menu, not the hidden .base state.
    if (browser.get() && browser->GetMainFrame()) {
      browser->GetMainFrame()->ExecuteJavaScript(
          ...
          "baseWidth:(b?b.getBoundingClientRect().width:-1)};"
    ...
    shareproof::Checkpoint("UI_OPEN_TRIGGERED", "openOSC via oscDisplayService");
    response = "true";
