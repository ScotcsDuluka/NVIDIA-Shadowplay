...
internal static class Program
{
        ...
        string evidenceDir, LoadGenerator? loadGen)
    {
        ...
        result.NoFrameReturnedCount = wgc.NoFrameReturnedCount;
        result.ShutdownDiscardedCount = wgc.ShutdownDiscardedCount;
    ...
    private static void ReportSession(SessionResult r)
    {
        ...
        Console.WriteLine($"    NoFrameReturned:     {r.NoFrameReturnedCount}");
        Console.WriteLine($"    ShutdownDiscarded:   {r.ShutdownDiscardedCount}");
        ...
        // Invariant: Acquired = Consumed + Dropped + ShutdownDiscarded
        long accounted = r.ConsumedFrameCount + r.DroppedByHarnessCount + r.ShutdownDiscardedCount;
        bool invariantHolds = r.AcquiredFrameCount == accounted;
        Console.WriteLine($"    Invariant (Acquired=Consumed+Dropped+ShutdownDiscarded): {(invariantHolds ? "HOLDS" : "MISMATCH")} " +
                          $"[{r.AcquiredFrameCount} == {r.ConsumedFrameCount} + {r.DroppedByHarnessCount} + {r.ShutdownDiscardedCount} = {accounted}]");
    }
}
